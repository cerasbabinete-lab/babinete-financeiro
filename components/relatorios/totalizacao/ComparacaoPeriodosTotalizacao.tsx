// ============================================================
// components/relatorios/totalizacao/ComparacaoPeriodosTotalizacao.tsx
// Projeto: Ceras Babinete — Gestão Financeira
// Módulo: Relatórios
// Função: Modo "Comparar períodos" do gráfico do relatório
//         Totalização (2.8) — dois seletores de data (Período A e
//         B), 100% manuais, sem sugestão automática (decisão
//         explícita da sessão de brainstorm). Não usa GraficoSvg —
//         ver nota em types/relatorios.ts (ComparacaoPeriodosResultado)
//         sobre por que este modo tem componente próprio.
// Conecta com: lib/relatorios/totalizacao.ts (compararPeriodosTotalizacao)
// ============================================================

'use client'

import { useState } from 'react'
import { compararPeriodosTotalizacao } from '@/lib/relatorios/totalizacao'
import { formatarMoeda } from '@/lib/relatorios/formatadores'
import { estilosRelatorio } from '@/components/relatorios/RelatorioUiComum'
import type { ComparacaoPeriodosResultado } from '@/types/relatorios'
...
export default function ComparacaoPeriodosTotalizacao() {
  ...
  const [resultado, setResultado] = useState<ComparacaoPeriodosResultado | null>(null)
  ...
  async function comparar() {
    ...
    setErro('')
    try {
      const r = await compararPeriodosTotalizacao(periodoA, periodoB)
      setResultado(r)
    } catch (err: unknown) {
    ...
    <div>
      ...
      {resultado && (
        <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
          ...
          <BlocoComparacao rotulo="Notas emitidas" valorA={resultado.notasA} valorB={resultado.notasB} formatoValor={v => String(v)} />
        </div>
      )}
    </div>
  )
